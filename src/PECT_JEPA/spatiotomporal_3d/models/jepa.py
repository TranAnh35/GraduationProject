...
class PECT_JEPA(nn.Module):
    ...
    def extract_dense_features(
        ...
        """
        Extract dense spatial representations at stride=1 (Dense Convolutional Patching).
        Produces full [H, W, T_c, D] or [H, W, D] native resolution (e.g. 300x300) without interpolation.
        ...
        Returns:
            [B, H, W, T_c, D] or [B, H, W, D] (or without batch dimension if input was 3D)
        """
        orig_ndim = clip.ndim
        if orig_ndim == 3:
            clip = clip.unsqueeze(0)

        # Dense patching at stride=1
        tokens_5d, (H, W, T_c) = self.tokenizer.forward_dense(clip)  # [B, H, W, T_c, D]

        if pool_temporal:
            tokens_5d = torch.mean(tokens_5d, dim=3)  # [B, H, W, D]

        if orig_ndim == 3:
            tokens_5d = tokens_5d.squeeze(0)
