...
class RepresentationAnomalyDetector:
    ...
    def fit(self, normal_features: torch.Tensor):
        """
        Fit baseline normal distribution from unlabelled/in-the-wild acquisition features.
        ...
        # Normalize features along embedding dimension D (spherical space)
        ...
        if self.method == "clustering":
            ...
            kmeans = MiniBatchKMeans(n_clusters=self.n_clusters, random_state=42, batch_size=2048).fit(feats_np)
            
            # Identify the dominant cluster (largest cluster is the homogeneous healthy metal)
            labels = kmeans.labels_
            counts = np.bincount(labels)
            dominant_cluster_id = int(np.argmax(counts))
            
            # Centroid of dominant healthy cluster
            ...
            # 2. Robust Coordinate-wise Median (breakdown point = 50% outliers)
