"""
Downstream Evaluation Script for PECT-JEPA.
Runs downstream evaluations using frozen pretrained encoder:
1. Anomaly Detection
...
from src.PECT_JEPA.data.dataset import find_all_tdms_files
from src.PECT_JEPA.evaluation.cross_sensor import evaluate_cross_sensor
from src.PECT_JEPA.evaluation.cross_wave import evaluate_cross_wave
from src.PECT_JEPA.evaluation.cross_liftoff import evaluate_cross_liftoff
...
def main():
    ...
    if args.task in ["all", "sensor"]:
        ...
        print("=" * 50)
        for target_sensor in ["TMR", "Hall_Pot_Core", "Differential_Pot_Core", "Hall_Air_Core"]:
    ...
    if args.task in ["all", "wave"]:
        ...
        print("=" * 50)
        for target_wave in ["Chirp", "Gaussian", "Square"]:
    ...
    if args.task in ["all", "liftoff"]:
        ...
        print("=" * 50)
        for target_liftoff in ["3mm", "2mm", "1mm"]:
