"""
Full-Scale Forward & Verification Benchmark for PECT-JEPA v0.2 (implement.md, Section 4.6).
Verifies complete forward and backward pass on full-scale clip [1, 300, 300, 16].
Grid: (37, 37, 16) = 21,904 tokens (D=128).
"""
...
from src.PECT_JEPA.training.optimizer import build_optimizer
...
class TestFullScaleForward(unittest.TestCase):
    def test_full_scale_300x300x16(self):
        ...
        # 2. Input clip: [1, 300, 300, 16]
        B, H, W, T_c = 1, 300, 300, 16
        clip = torch.randn(B, H, W, T_c, device=device)
        print(f"1. Input Clip Shape: {clip.shape} ({clip.numel() * 4 / (1024**2):.2f} MB)")
        ...
        self.assertEqual(grid_shape, (37, 37, 16))
        self.assertEqual(total_tokens, 21904)
        self.assertEqual(tgt_idx1.shape[1], 8 * 8 * 8)  # 512 target tokens
        ...
        self.assertEqual(feats.shape, (1, 37, 37, 16, 128))
        print(f"6. Full 4D Feature Extraction: {feats.shape} (pool_temporal=False): SUCCESS")

        print("=" * 70)
        print("FULL-SCALE CLIP BENCHMARK PASSED 100%!")
        print("=" * 70)
