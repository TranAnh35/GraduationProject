"""
Unified Experiment Tracker & Logger for 1D Temporal PECT-JEPA.
...
class PECTExperimentLogger:
    """
    State-of-the-art multi-tier experiment logger for PECT-JEPA.
    """
    ...
    def __init__(self, config: Any, rank: int = 0):
        ...
        exp_name = getattr(config, "exp_name", "jepa_1d_base")
        log_base_dir = getattr(config, "log_dir", "experiments/1d")
        ...
        self.logger = logging.getLogger(f"PECT_JEPA_1D_{self.run_name}")
        ...
        if self.is_main:
            ...
            # File Formatter
            f_formatter = logging.Formatter("[%(asctime)s | %(levelname)s | %(filename)s:%(lineno)d] %(message)s", datefmt="%Y-%m-%d %H:%M:%S")
        ...
        if self.use_wandb:
            wandb_proj = getattr(config, "wandb_project", "PECT_JEPA_1D")
        ...
        if self.is_main:
            ...
            self.step_csv_file = open(step_csv_path, "w", newline="", encoding="utf-8")
            ...
            self.info(f"=== PECT-JEPA Experiment Initialized: {self.run_name} ===")
    ...
    def log_epoch(self, epoch: int, metrics: Dict[str, Any], step: int = 0):
        """Logs epoch-level aggregated metrics (val loss, effective rank, time)."""
        if not self.is_main:
            return
        ...
        if self.tb_writer:
            for k, v in metrics.items():
                if isinstance(v, (int, float, np.floating, np.integer)):
                    prefix = "val" if "val" in k or "effective_rank" in k else "train"
    ...
    def log_model_histograms(self, model: nn.Module, global_step: int):
        """Logs model weights and gradient distributions to TensorBoard."""
        if not self.is_main or not self.tb_writer:
            return
        for name, param in model.named_parameters():
            if param.requires_grad and param.data is not None:
                self.tb_writer.add_histogram(f"weights/{name}", param.data.cpu().numpy(), global_step)
                if param.grad is not None:
                    self.tb_writer.add_histogram(f"gradients/{name}", param.grad.data.cpu().numpy(), global_step)
